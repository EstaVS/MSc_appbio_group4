...
The first step was to setup the work environment and data on the HPC and where we navigated into our scratch place to start an interactive job.

1.  Log into the CREATE node and start an interactive job

```{bash}
/scratch_tmp/grp/msc_appbio/group4_tmp/ #navigating to the scratch directory
...
2.  Create new directories to store files
...
3.  Install and activate the tools needed for environment setup using Conda in the environment.sh nano file
...
The output should be .sortedByCoord.out.bam files for the 20 fastq files
...
# -s yes specifies that the RNA-Seq data is strand specific
	 ...
	 # Check if HTSeq-Count ran successfully
        if [[ $? -eq 0 ]]; then # makes sure that htseq-count ran successfully and whether any errors were found
...
2.  Reading and loading the count files and metadata
...
3.  Extracting and Saving gene-level annotations from GTF file in a GTF format
...
#Read the gene_ids.txt into a new data frame 
reference_genes <- read.table("gene_ids.txt", header = FALSE, stringsAsFactors = FALSE)

#Extracts first column from reference_genes, containing gene_ids
reference_genes <- reference_genes$V1 
```

4.  Aligning and Processing Gene count data to the reference genome
...
6.  Creating a DeSeq2 dataset and filtering low count genes
...
#Filtering low-count genes to those with at least 10 reads in at least 2 samples 
dds <- dds[rowSums(counts(dds) >= 10) >= 2, ]
```

7.  Normalize the data and handle missing values from differential gene analysis
...
# Identify where the missing values are from
sum(is.na(results$pvalue)) # looks at NAs in p-value (5 NAs)

# Excluding missing values (NA) from analysis
exNA_results <- results[!is.na(results$pvalue), ]
head(exNA_results)

#Confirming whether NA values have been excluded
sum(is.na(exNA_results))
sum(is.na(exNA_results$padj))
```

# Adjusting the P-values with the FDR tool for Multiple Testing Correction

For differential gene expression analysis in RNA-Seq, each gene will be tested for statistical significance. Loads of statistical tests are performed and these multiple tests should be corrected for. If not, the number of genes being highlighted as differentially expressed will be inaccurate, leading to incorrect conlusions and invalid downstream analysis.
...
3.  Saving the data in a portable format like a csv file

```{r}
#Saving differential expression analysis data to a csv file
...
```{r}
#Convert significant genes csv into an RDS object

# Read the significant genes csv from DeSeq2 analysis into R
results_df <- read.csv("significant_genes_results.csv", row.names = 1)
...
5.  Extract Transformed Data for the Heatmap
...
6.  Selecting the 50 most highly expressed genes from the rld_matrix dataset
...
rownames(sample_annotation) <- colnames(rld_matrix) #sets rownames to align with column names of rld_matrix
...
## Create a PCA (Principlal Component Analysis) plot

This is vital for RNA-Seq analysis, showing how the samples cluster depending on their gene expression patterns. So those with similar gene expression patterns will cluster together.
...
```{r}
# Use count_matrix data for PCA data
head(count_matrix)

# Establishing PCA variability across columns 
zero_var_cols <- apply(count_matrix, 1, function(x) var(x) == 0) #identifies rows where variance=0 and removes them
...
# Showing string of phenotype metadata and strain metadata
metadata$phenotype
metadata$strain
```
...
PCAplot <- ggplot(pca, aes(x = PC1, y = PC2, colour = metadata$phenotype, shape = metadata$strain)) + #PC1 and PC2 are set as the axes with color based on phenotype and shape based on strain
         ...
         labs(title = "Transcriptomics PCA Plot", x = "PC1: 35.5% variance", y = "PC2: 28.6% variance", colour = "Phenotype", shape = "Strain Type") + #add titles and axes labels, explaining %variance in PC1 and 2
